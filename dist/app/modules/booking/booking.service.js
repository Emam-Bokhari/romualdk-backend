...
// -------- Create Booking ----------
const createBooking = (body, userId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { billableDays } = (0, bookingCalculation_1.calculatePrice)(fromDate, toDate);
...
// };
// =======================MOSHFIQUR RAHMAN====================
const getUserBookings = (userId, status) => __awaiter(void 0, void 0, void 0, function* () {
    const filter = { userId };
    if (status)
        filter.carStatus = status;
    ...
    const carIds = bookings
        .map((booking) => { var _a; return (_a = booking.carId) === null || _a === void 0 ? void 0 : _a._id; })
        .filter(Boolean)
        .map((id) => new mongoose_1.Types.ObjectId(id));
    // ---------- STEP 3: Trip count ----------
    const tripCountMap = yield (0, car_utils_1.getCarTripCountMap)(carIds);
    // ---------- STEP 4: Attach trips + rating ----------
    const finalBookings = yield Promise.all(bookings.map((booking) => __awaiter(void 0, void 0, void 0, function* () {
        var _a, _b;
        const carId = (_b = (_a = booking.carId) === null || _a === void 0 ? void 0 : _a._id) === null || _b === void 0 ? void 0 : _b.toString();
        ...
        return Object.assign(Object.assign({}, booking), { carId: Object.assign(Object.assign({}, booking.carId), { trips: tripCountMap[carId] || 0, averageRating: reviewSummary.averageRating, totalReviews: reviewSummary.totalReviews, starCounts: reviewSummary.starCounts, reviews: reviewSummary.reviews }) });
    })));
...
// };
// =======================MOSHFIQUR RAHMAN====================
const getHostBookings = (hostId, status) => __awaiter(void 0, void 0, void 0, function* () {
    const filter = { hostId };
    if (status)
        filter.carStatus = status;
    // ---------- STEP 1: Fetch bookings ----------
...
// =======================MOSHFIQUR RAHMAN====================
const checkIn = (bookingId) => __awaiter(void 0, void 0, void 0, function* () {
...
const checkOut = (bookingId) => __awaiter(void 0, void 0, void 0, function* () {
    const booking = yield booking_model_1.Booking.findById(bookingId);
    if (!booking)
        throw new Error("Booking not found");
    if (!booking.checkIn)
...
//   return booking;
// };
const isCancelled = (bookingId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (booking.status === booking_interface_1.BOOKING_STATUS.PAID) {
            if (booking.payoutProcessed)
                throw new Error("Refund not allowed after host payout");
            const transaction = yield transaction_model_1.default.findById(booking.transactionId).session(session);
            if (!transaction)
                throw new Error("Transaction not found");
            const refundPercentage = (0, refundCalculation_1.calculateRefundPercentage)(booking.fromDate);
            if (refundPercentage === 0)
                throw new Error("Refund not applicable");
            // Stripe call
            yield payment_service_1.PaymentService.refundBookingPayment(booking, transaction, refundPercentage, session);
        }
        booking.isCancelled = true;
...
const updateBookingByAdmin = (bookingId, payload) => __awaiter(void 0, void 0, void 0, function* () {
    if (!mongoose_1.Types.ObjectId.isValid(bookingId))
        throw new Error("Invalid booking id");
    const booking = yield booking_model_1.Booking.findByIdAndUpdate(bookingId, payload, {
        new: true,
    })
        .populate("carId")
        .populate("userId")
...
const deleteBookingByAdmin = (bookingId) => __awaiter(void 0, void 0, void 0, function* () {
    if (!mongoose_1.Types.ObjectId.isValid(bookingId))
        throw new Error("Invalid booking id");
...
const getBookingStatusStats = (year) => __awaiter(void 0, void 0, void 0, function* () {
                ...
                analyticsDate: {
                    $switch: {
                        branches: [
                            ...
                            {
                                case: { $eq: ["$carStatus", booking_interface_1.CAR_STATUS.CANCELLED] },
                                then: "$cancelledAt",
                            },
                            // Completed → checkOut / toDate
                            {
                                case: { $eq: ["$carStatus", booking_interface_1.CAR_STATUS.COMPLETED] },
                                then: "$checkedOutAt",
                            },
                            // Upcoming / Active → fromDate
                            {
                                case: {
                                    $in: [
                                        "$carStatus",
                                        [booking_interface_1.CAR_STATUS.UPCOMING, booking_interface_1.CAR_STATUS.ONGOING],
                                    ],
                                },
                                then: "$fromDate",
                            },
                ...
                chartStatus: {
                    $switch: {
                        branches: [
                            ...
                            {
                                case: { $eq: ["$carStatus", booking_interface_1.CAR_STATUS.ONGOING] },
                                then: "Active",
                            },
                            {
                                case: { $eq: ["$carStatus", booking_interface_1.CAR_STATUS.CANCELLED] },
                            ...
                            },
                            {
                                case: {
                                    $and: [
                                        { $eq: ["$status", booking_interface_1.BOOKING_STATUS.PAID] },
                                        { $eq: ["$checkIn", false] },
                                    ],
                                },
                                then: "Upcoming",
