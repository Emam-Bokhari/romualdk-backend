...
const createCarToDB = (userId, payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (error.code === 11000) {
            throw new ApiErrors_1.default(400, "Car with this license plate already exists");
        }
        throw new ApiErrors_1.default(error.statusCode || 500, error.message || "Failed to create a car");
    }
});
// for feed
...
//   return result;
// };
const getAllCarsFromDB = (query, userId) => __awaiter(void 0, void 0, void 0, function* () {
    const { searchTerm, minPrice, maxPrice, transmission, fuelType, color, city, rating, latitude, longitude, maxDistance, withDriver, date, time, sort, page = 1, limit = 10 } = query;
    // reusable function get location
    const { lat, lng } = yield (0, car_utils_1.getTargetLocation)(latitude, longitude, userId);
    // ---------- create dynamic object ----------
    const filter = {
    ...
    if (minPrice || maxPrice) {
        filter.dailyPrice = {};
        if (minPrice)
            filter.dailyPrice.$gte = Number(minPrice);
        if (maxPrice)
    ...
    const processedCars = yield Promise.all(cars.map((carData) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (!car)
            return null;
        // Availability check
        const availability = yield exports.CarServices.getAvailability(car._id.toString(), targetDate);
        ...
        // date or time filter
        if ((date || time) && !isAvailable)
            return null;
        // rating filter
        const reviewSummary = yield review_service_1.ReviewServices.getReviewSummaryFromDB(car._id.toString(), review_interface_1.REVIEW_TYPE.CAR);
        if (rating && reviewSummary.averageRating < Number(rating))
            return null;
        // other data
        const tripsCountMap = yield (0, car_utils_1.getCarTripCountMap)([car._id]);
        const isBookmarked = yield favouriteCar_model_1.FavouriteCar.exists({ userId, referenceId: car._id });
        return Object.assign(Object.assign({}, car.toObject()), { 
            // distance in km
            distance: carData.distance !== undefined ? (carData.distance / 1000).toFixed(1) : "0.0", isAvailable,
            availabilityCalendar, trips: tripsCountMap[car._id.toString()] || 0, isFavourite: Boolean(isBookmarked), averageRating: reviewSummary.averageRating, totalReviews: reviewSummary.totalReviews, starCounts: reviewSummary.starCounts, reviews: reviewSummary.reviews, availabilitySlots: availability.slots });
    })));
    // filter out nulls
    const finalCars = processedCars.filter(car => car !== null);
    const result = {
...
//   };
// };
const getRecentCarsFromDB = (userId) => __awaiter(void 0, void 0, void 0, function* () {
    const limit = 10;
    //  get location
    const { lat, lng } = yield (0, car_utils_1.getTargetLocation)(undefined, undefined, userId);
    ...
    const result = yield Promise.all(cars.map((carData) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        return Object.assign(Object.assign({}, carWithUser), { 
            ...
            distance: carData.distance !== undefined ? (carData.distance / 1000).toFixed(1) : "0.0", isFavourite: Boolean(favouriteMap[carData._id.toString()]) });
    })));
    return {
        data: result,
        meta: {
...
const getAllCarsForVerificationsFromDB = (query) => __awaiter(void 0, void 0, void 0, function* () {
...
const updateCarVerificationStatusByIdToDB = (carId, carVerificationStatus) => __awaiter(void 0, void 0, void 0, function* () {
    if (![
        car_interface_1.CAR_VERIFICATION_STATUS.PENDING,
        car_interface_1.CAR_VERIFICATION_STATUS.APPROVED,
        car_interface_1.CAR_VERIFICATION_STATUS.REJECTED,
    ].includes(carVerificationStatus)) {
        throw new ApiErrors_1.default(400, "Car verification status must be either 'PENDING','APPROVED' or 'REJECTED'");
    }
    console.log(carVerificationStatus, "STATUS");
    const result = yield car_model_1.Car.findByIdAndUpdate(carId, { verificationStatus: carVerificationStatus }, { new: true });
    if (!result) {
        throw new ApiErrors_1.default(400, "Failed to change car verification status by this car ID");
    }
    return result;
});
...
//   };
// };
const getOwnCarsFromDB = (_a) => __awaiter(void 0, [_a], void 0, function* ({ userId, verificationStatus, }) {
    const user = yield user_model_1.User.findOne({
        _id: userId,
        role: user_1.USER_ROLES.HOST,
    });
    ...
    const carsWithMeta = yield Promise.all(cars.map((carData) => __awaiter(void 0, void 0, void 0, function* () {
        const car = yield car_model_1.Car.findById(carData._id).populate({
            path: "userId",
            select: "firstName lastName fullName role profileImage email phone",
        });
        ...
        return Object.assign(Object.assign({}, car.toObject()), { 
            // convert distance to km 
            distance: carData.distance !== undefined ? (carData.distance / 1000).toFixed(1) : "0.0", isFavourite: Boolean(isBookmarked), averageRating: reviewSummary.averageRating, totalReviews: reviewSummary.totalReviews, starCounts: reviewSummary.starCounts, reviews: reviewSummary.reviews });
    })));
    // null filter
    return carsWithMeta.filter(car => car !== null);
});
const getCarByIdFromDB = (id, userId) => __awaiter(void 0, void 0, void 0, function* () {
    // get location
    const { lat, lng } = yield (0, car_utils_1.getTargetLocation)(undefined, undefined, userId);
    const cars = yield car_model_1.Car.aggregate([
        {
    ...
    const availabilityCalendar = yield (0, car_utils_1.getCarCalendar)(id.toString());
    const reviewSummary = yield review_service_1.ReviewServices.getReviewSummaryFromDB(id, review_interface_1.REVIEW_TYPE.CAR);
    const trips = yield (0, car_utils_1.getCarTripCount)(id);
    return Object.assign(Object.assign({}, car.toObject()), { 
        // convert distance to km
        distance: carData.distance !== undefined ? (carData.distance / 1000).toFixed(1) : "0.0", trips: trips || 0, isAvailable,
        availabilityCalendar, isFavourite: Boolean(isBookmarked), averageRating: reviewSummary.averageRating, totalReviews: reviewSummary.totalReviews, starCounts: reviewSummary.starCounts, reviews: reviewSummary.reviews });
});
...
const getAvailability = (carId, dateString) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const bookings = yield booking_model_1.Booking.find({
        ...
        status: { $in: [booking_interface_1.BOOKING_STATUS.PAID, booking_interface_1.BOOKING_STATUS.ONGOING] },
...
//   return finalCars;
// };
const getSuggestedCarsFromDB = (userId_1, ...args_1) => __awaiter(void 0, [userId_1, ...args_1], void 0, function* (userId, limit = 10) {
    console.log("===== START getSuggestedCarsFromDB =====");
    const { lat, lng } = yield (0, car_utils_1.getTargetLocation)(undefined, undefined, userId);
    ...
    for (const car of rawCars) {
        console.log("Checking car:", car._id);
        const isAvailable = yield (0, car_utils_1.checkCarAvailabilityByDate)(car, targetDate);
        const availabilityCalendar = yield (0, car_utils_1.getCarCalendar)(car._id.toString());
        if (isAvailable) {
            suggestedCars.push(Object.assign(Object.assign({}, car), { isAvailable: true, availabilityCalendar }));
        }
        if (suggestedCars.length === limit)
            break;
    }
    console.log("Suggested cars after availability check:", suggestedCars.length);
    ...
    const finalCars = yield Promise.all(populatedCars.map((car) => __awaiter(void 0, void 0, void 0, function* () {
        const reviewSummary = yield review_service_1.ReviewServices.getReviewSummaryFromDB(car._id, review_interface_1.REVIEW_TYPE.CAR);
        return Object.assign(Object.assign({}, car), { trips: tripCountMap[car._id.toString()] || 0, averageRating: reviewSummary.averageRating, totalReviews: reviewSummary.totalReviews, starCounts: reviewSummary.starCounts, reviews: reviewSummary.reviews });
    })));
    console.log("===== END getSuggestedCarsFromDB =====");
    return finalCars;
});
...
// =====================================================
const isCarBookableForDay = (car, date) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    const dayStart = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()));
    const dayEnd = new Date(dayStart.getTime() + 86400000);
    // Manual full-day block
    const isManuallyBlocked = (_a = car.blockedDates) === null || _a === void 0 ? void 0 : _a.some((b) => {
    ...
    if (isManuallyBlocked)
        return false;
    // AvailableDays check
    if ((_b = car.availableDays) === null || _b === void 0 ? void 0 : _b.length) {
...
const getCarsByDestinationFromDB = (destinationId) => __awaiter(void 0, void 0, void 0, function* () {
    // destination find
    const destination = yield destination_model_1.Destination.findById(destinationId);
    if (!destination) {
